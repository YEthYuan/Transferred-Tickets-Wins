...
pydir = r"~/anaconda3/envs/pytorch/bin/python"
cuda = "0"  # "0,1,2"
arch = "ResNet50"
constraint = "L2"
adv_eps = 0.05
run_mode = 4  # 0: 0.1~0.9  1: 0.91~0.99  2: all rate  3: only zero  4: custom rate
custom_rate = [x / 10 for x in range(1, 5, 1)]
...
for local_dict in ll[adv_nat]:
    ...
    if eps == 0:
        ...
        if constraint == "L2":
            inner = f"--model_path ~/ResNet_ckpt/L2/{arch.lower()}_{eps_layout}.ckpt"
        elif constraint == "Linf":
            inner = f"--model_path ~/ResNet_ckpt/{arch.lower()}_{eps_layout}.ckpt"

    for rate in rl[run_mode]:
        start = time.time()
        exp_name = f"{eps_layout}-sp{rate}"

        shell_code = f"{pydir} main_transfer_imagenet.py --gpu_id {cuda} --eps {adv_eps} {inner} --name {exp_name} --prune_method omp --prune_rate {rate}"
