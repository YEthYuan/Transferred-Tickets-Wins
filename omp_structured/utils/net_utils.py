...
def save_checkpoint(state, is_best, filename="checkpoint.pth", save=False):
    ...
    shutil.copyfile(filename, str(filename.parent / "model_latest.pth"))
...
def init_model_weight_with_score(model, prune_rate):
    print(f"==> Initialize model weights with score based on pruning rate {1 - prune_rate:.2f}")

    for n, m in model.named_modules():
        if hasattr(m, "init_weight_with_score"):
            m.init_weight_with_score(prune_rate)
