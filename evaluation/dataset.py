...
def cifar10_dataloaders(args, use_val=True, norm=True):
    if norm:
        ...
        test_transform = transforms.Compose([
            ...
            AddGaussianNoise(mean=0, variance=1, amplitude=args.amplitude),
            transforms.ToTensor(),
            normalize
        ])
        ...
        orig_test_transform = transforms.Compose([
            # transforms.Resize(32),
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
        ...
        orig_test_set = CIFAR10(args.data, train=False, transform=orig_test_transform, download=True)
        orig_test_loader = DataLoader(orig_test_set, batch_size=args.batch_size, shuffle=False,
        ...
        orig_test_set = CIFAR10(args.data, train=False, transform=orig_test_transform, download=True)
        orig_test_loader = DataLoader(orig_test_set, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                                 pin_memory=True)

        dataset_normalization = NormalizeByChannelMeanStd(mean=[0.4914, 0.4822, 0.4465], std=[0.2023, 0.1994, 0.2010])

    return orig_test_loader, dataset_normalization, test_loader
...
def cifar100_dataloaders(args, use_val=True, norm=True):
    if norm:
        ...
        orig_test_set = CIFAR100(args.data, train=False, transform=orig_test_transform, download=True)
        orig_test_loader = DataLoader(orig_test_set, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                                 pin_memory=True)
...
def svhn_dataloaders(args, use_val=True, norm=True):
    if norm:
        ...
        orig_test_set = SVHN(args.data, split='test', transform=orig_test_transform, download=True)
        orig_test_loader = DataLoader(orig_test_set, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                                 pin_memory=True)
...
def imagenet_dataloaders(args, use_val=False, norm=True):
    ...
    return orig_val_loader, dataset_normalization, val_loader
